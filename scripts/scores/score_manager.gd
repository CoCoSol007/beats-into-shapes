...
signal apply_level_score(score: int)
...
func on_node_pressed(node: BeatEvent, _time: float, status: Constants.ActionStatus):
	...
	handle_status(status, node.is_bad)

# Called when a beat event node is released
func on_node_released(node: BeatEvent, _time: float, status: Constants.ActionStatus):
	...
	handle_status(status, false)

# Called when a press is missed
func on_missed_press(_time: float, _key: Constants.ActionKey):
	# Handle the missed press status
	handle_status(Constants.ActionStatus.MISSED, false)

# Function to handle the status of a press or release
func handle_status(status: Constants.ActionStatus, is_bad: bool):
	...
	match status:
		...
		Constants.ActionStatus.PERFECT:
			combo += 1
			score += PERFECT_SCORE * min(combo, MAX_COMBO_BONUS)
			if combo >= 2:
				show_status("Perfect ×" + str(combo), Color(59 / 255.0, 126 / 255.0, 79 / 255.0))
			else:
				show_status("Perfect", Color(59 / 255.0, 126 / 255.0, 79 / 255.0))
		
		Constants.ActionStatus.PERFECT_PLUS:
			combo += 1
			combo_plus += 1
			score += PERFECT_PLUS_SCORE * min(combo, MAX_COMBO_BONUS)
			if combo >= 2:
